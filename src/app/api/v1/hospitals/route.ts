import { NextResponse } from 'next/server';
import { supabase } from '@/lib/supabaseClient';
import { uniqueHospitalNames } from '@/lib/normalizeHospital';
import { withCors, CORS_HEADERS } from '@/lib/cors';
import { checkApiRateLimit } from '@/lib/apiRateLimit';
import { getClientIp } from '@/lib/rateLimit';

// Lista pública de hospitales/centros (para poblar un filtro desde otra web).
export async function OPTIONS() {
  return new Response(null, { status: 204, headers: CORS_HEADERS });
}

export async function GET(request: Request) {
  const ip = getClientIp(request);
  const limit = checkApiRateLimit(ip);
  if (!limit.allowed) {
    return NextResponse.json(
      { error: 'Demasiadas peticiones. Intenta de nuevo en unos segundos.' },
      { status: 429, headers: withCors({ 'Retry-After': String(limit.retryAfter) }) }
    );
  }

  try {
    // Traemos TODOS los valores de hospital en bloques de 1000 (límite de Supabase).
    const CHUNK = 1000;
    const MAX_CHUNKS = 50;
    const valores: (string | null)[] = [];

    for (let i = 0; i < MAX_CHUNKS; i++) {
      const from = i * CHUNK;
      const { data, error } = await supabase
        .from('personas_rescatadas')
        .select('hospital')
        .not('hospital', 'is', null)
        .neq('hospital', '')
        .order('hospital', { ascending: true })
        .range(from, from + CHUNK - 1);

      if (error) throw error;

      const batch = data || [];
      valores.push(...batch.map((r) => r.hospital as string));
      if (batch.length < CHUNK) break;
    }

    const uniqueHospitals = uniqueHospitalNames(valores).sort((a, b) =>
      a.localeCompare(b, 'es', { sensitivity: 'base' })
    );

    return NextResponse.json(
      { data: uniqueHospitals },
      { headers: withCors({ 'Cache-Control': 'public, max-age=60' }) }
    );
  } catch (err) {
    console.error('Error en /api/v1/hospitals:', err);
    return NextResponse.json(
      { error: 'Error interno del servidor' },
      { status: 500, headers: CORS_HEADERS }
    );
  }
}
